                                   ...
                                   uint id [[thread_position_in_grid]])
{
    ...
    const float distanceZeroA = fast::max(distance_squared(particlePositionAFloat, gravityWellZeroPosition), dist_sq);
    const float distanceOneA = fast::max(distance_squared(particlePositionAFloat, gravityWellOnePosition), dist_sq);
    const float distanceTwoA = fast::max(distance_squared(particlePositionAFloat, gravityWellTwoPosition), dist_sq);
    const float distanceThreeA = fast::max(distance_squared(particlePositionAFloat, gravityWellThreePosition), dist_sq);
    ...
    const float distanceZeroB = fast::max(distance_squared(particlePositionBFloat, gravityWellZeroPosition), dist_sq);
    const float distanceOneB = fast::max(distance_squared(particlePositionBFloat, gravityWellOnePosition), dist_sq);
    const float distanceTwoB = fast::max(distance_squared(particlePositionBFloat, gravityWellTwoPosition), dist_sq);
    const float distanceThreeB = fast::max(distance_squared(particlePositionBFloat, gravityWellThreePosition), dist_sq);
    ...
    const float distanceZeroC = fast::max(distance_squared(particlePositionCFloat, gravityWellZeroPosition), dist_sq);
    const float distanceOneC = fast::max(distance_squared(particlePositionCFloat, gravityWellOnePosition), dist_sq);
    const float distanceTwoC = fast::max(distance_squared(particlePositionCFloat, gravityWellTwoPosition), dist_sq);
    const float distanceThreeC = fast::max(distance_squared(particlePositionCFloat, gravityWellThreePosition), dist_sq);
    ...
    const float distanceZeroD = fast::max(distance_squared(particlePositionDFloat, gravityWellZeroPosition), dist_sq);
    const float distanceOneD = fast::max(distance_squared(particlePositionDFloat, gravityWellOnePosition), dist_sq);
    const float distanceTwoD = fast::max(distance_squared(particlePositionDFloat, gravityWellTwoPosition), dist_sq);
    const float distanceThreeD = fast::max(distance_squared(particlePositionDFloat, gravityWellThreePosition), dist_sq);
